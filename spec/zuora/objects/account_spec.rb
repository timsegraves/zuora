  ...
  it "has defined attributes" do
    subject.attributes.keys.map(&:to_s).sort.should ==
      ["account_number", "additional_email_addresses", "allow_invoice_edit", "auto_pay", "balance",
       "batch", "bcd_setting_option", "bill_cycle_day", "bill_to_id", "communication_profile_id", "coupon_code__c",
       ...
       "invoice_template_id", "is_reseller__c", "last_invoice_date", "name", "notes", "parent_id", "payment_gateway", "payment_term",
       "purchase_order_number", "reseller_account_id__c", "sales_rep_name", "sold_to_id", "status", "updated_by_id", "updated_date"]
  end
